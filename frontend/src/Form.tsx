...
export default class Form extends Component<Props, State> {
    ...
    handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (msg.startsWith("/")) {
            handleSlashCommand.call(this.state.thisMsg, { username: this.props.username, room: this.props.room, msg: this.state.message, password: this.props.password });
    ...
    setupSocketEventListeners = () => {
        ...
        this.socket.on('message-history', (dat: any) => {
            /* let msgses = []
            for(let ms of dat) {
                if(ms.password === this.props.password && ms.room === this.props.room) {
                    msgses.push(ms)
                }
            } */
        ...
        this.socket.emit('join_room', { room: this.props.room, password: this.props.password });
        this.socket.on('join_room', (msg: any) => {
            this.setState({ joinedRoomOrNot: msg }, () => {this.props.isjoined(this.state.joinedRoomOrNot.text) })
        })
    }
    ...
    displayMessageHistory() {
        if (this.state.messages !== null && this.state.messages !== undefined && this.state.messages.length > 0) {
            return this.state.messages.map((message: { username: string, id: number, room: string, messages: string[] }) => {

                if ( this.state.messages !== null && this.state.messages !== undefined && message.messages.length > 0) {
    ...
    test = ()=>{
        console.log(this.state.joinedRoomOrNot)
        if(this.state.joinedRoomOrNot.text ==='success') {
        return (
